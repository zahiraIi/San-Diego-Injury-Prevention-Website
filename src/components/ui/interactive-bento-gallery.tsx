            ...
            >
                ...
                <div className="relative h-full flex flex-col items-center justify-center p-4">
                    <AnimatePresence mode="wait">
                        <motion.div
                            key={selectedItem.id}
                            className="relative w-full aspect-[16/9] max-w-[98%] sm:max-w-[95%] md:max-w-6xl 
                                     h-auto max-h-[90vh] rounded-lg overflow-hidden shadow-2xl"
                    ...
                    <motion.button
                        className="absolute top-4 sm:top-5 md:top-6 right-4 sm:right-5 md:right-6 
                                  p-3 sm:p-3.5 md:p-4 rounded-full bg-white/90 text-gray-700 hover:bg-white 
                                  shadow-lg z-10"
                        onClick={(e) => {
                            e.stopPropagation();
                            onClose();
                    ...
                    <div className="flex items-center -space-x-2.5 px-3.5 py-2.5">
                        {mediaItems.map((item, index) => (
                            <motion.div
                                ...
                                className={`
                                    relative group
                                    w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 flex-shrink-0 
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems, title, description, hideHeader = false }) => {
        ...
        <div className="container mx-auto px-2 md:px-4 py-4 max-w-7xl">
            ...
            <AnimatePresence mode="wait">
                {selectedItem ? (
                    ...
                    <motion.div
                        className="grid grid-cols-4 gap-2 sm:gap-3 md:gap-4 auto-rows-[40px] sm:auto-rows-[60px] md:auto-rows-[100px] grid-flow-dense scale-85 sm:scale-90 md:scale-100 origin-top"
                        initial="hidden"
                        animate="visible"
                        exit="hidden"
                        variants={{
